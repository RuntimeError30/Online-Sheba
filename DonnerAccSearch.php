...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM bloodeligibility WHERE BG = '$BG' AND District = '$DISTRICT'";
    
    $searchShow = mysqli_query($conn,$query);
...
<body style="background-image: url('folder2/bgPng1.png');" class="bg-[#fffbff]">
    ...
    <main style="font-family: 'CharukolaUltraLight', sans-serif; " class="flex-col mx-16 px-9 py-2 my-11">
        ...
        <div class="py-20">
        <form action="donnerNoAccSearch.php" method="POST">
            <div class="flex">
...
<tr >
    ...
    <th class="border-2 px-3">অ্যাকশন</th>
      ...
      <!-- populate table from mysql database -->
                <?php while($Trow = mysqli_fetch_array($searchShow)):?>
                    <tr>

        <td class="border-2 px-3"><?php echo $Trow['Name']?> </td>
        <td class="border-2 px-3"><?php echo $Trow['BG']?> </td>
        <td class="border-2 px-3"><?php echo $Trow['Mobile']?> </td>
        <td class="border-2 px-3"><?php echo $Trow['Division']?> </td>
        <td class="border-2 px-3"><?php echo $Trow['District']?> </td>
        <td class="border-2 px-3"><?php echo $Trow['Eligibility']?> </td>
        <td class="border-2 px-3"><a href="urgentMessage.php?ID=<?php echo $Trow['ID']?>" class="">নোটিফিকেশন পাঁঠান</a></td>
