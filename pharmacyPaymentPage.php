...
if(!empty($_SESSION["Email"])){
  ...
  $productFetcher = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM confirmorders  WHERE Email = '$Email' AND BuyStatus = 'Not Bought'"));
...
if(isset($_POST["OrderConfirmed"])){
    ...
    if($Payment == "Bkash"){
        ...
        $Product = $productFetcher["OrderProd"];
        ...
        $PlaceOrderQ = mysqli_query($conn,"UPDATE confirmorders SET PaymentStatus='$PaymentStatus',PaymentMethod='$method',BuyStatus='Bought', Time = '$Time' WHERE Email = '$Email'");
...
font-family: 'Raleway', sans-serif;" >
                ...
                $Date = date("y-m-d");
                        ...
                        $result = $conn->query($query1);
                            ...
                            <tr>
                                    ...
                                    <td class="border-2 px-3 py-7"> <a href="removeCart.php?ID=<?php echo $Cartrow['ID']?>&&ProductID=<?php echo $Cartrow['ProductID']?>&&Email=<?php echo $Cartrow['Email']?>&&OrderID=<?php echo $Cartrow["OrderID"]?>" class="mt-10 shadow-xl bg-yellow-400 px-1 py-2">কার্ট থেকে রিমুভ করুন</a></td>
    ...
    <div class="form-check">
      ...
      <input type="submit" name="OrderConfirmed" value="অর্ডার কনফার্ম করুন" style="font-family: 'CharukolaUltraLight', sans-serif;" class="px-28  bg-gradient-to-l from-[#c04848] to-[#480048] drop-shadow-2xl text-white font-extrabold py-4 mt-6 border-none mx-5 hover:rounded-2xl hover:shadow-2xl hover:cursor-pointer hover:scale-110 hover:bg-gradient-to-r from-[#c04848] to-[#480048] transition-all ease-out duration-700"><br><br>
            ...
            <div>
                <a href="pharmacyClient.php" style="font-family: 'Hind Siliguri', sans-serif;" class="flex relative right-20">কার্টে আরও পন্য যোগ করুন</a>
            </div>
