...
$emptyCheck = (mysqli_fetch_assoc(mysqli_query($conn,$emptyQ)));
    ...
    if(isset($_POST['rateBtn'])){
        ...
        if(($emptyCheck["ID"] == $Prow["DonnerID"]) &&  ($emptyCheck["HosID"] == $_GET["ID"])){
...
<body class="bg-gray-100" style="font-family: 'CharukolaUltraLight', sans-serif;">
    <nav class="bg-gradient-to-b from-[#480048] to-[#480048] text-white flex justify-between py-4 px-9 border-spacing-0 border-b-0 h-56">
        ...
        <a href="BloodDonnerdashboard.php"style="font-family: 'CharukolaUltraLight', sans-serif;" class="flex"><img src='folder2/<?php echo $Prow["ProfilePhoto"]; ?>' class=" relative w-20 h-20 py-1 border-2 -left-10 top-3 rounded-lg scale-50 "></a>
    </nav>
    <main class="">
            ...
            <div class="shadow-2xl bg-gray-100 w-fit h-fit">
                ...
                <div id="sidebar" class="grid  justify-start absolute -left-3 top-72 bg-gradient-to-b from-[#2c2c2c] to-[#181818] w-fit h-fit p-5 rounded-r-xl scale-90">
                    <ul class="space-y-2 text-sm">
                        <a href="DocAppointmentBloodDonners.php?ID=<?php echo $row['ID']?>"  class="font-semibold text-lg text-gray-700 capitalize font-poppins tracking-wide hover:text-white transition-all">
                    ...
                    <ul class="space-y-2 text-sm">
                    <a href="bloodBankBookBloodDonners.php?ID=<?php echo $row['ID']?>" class="font-semibold text-lg text-gray-700 capitalize font-poppins tracking-wide">
