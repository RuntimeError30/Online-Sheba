<?php
    ...
    $result =  mysqli_query($conn, "SELECT * FROM generalmemberlist WHERE Email = '$Email' OR NewPassword = '$NewPassword'");
    $row = mysqli_fetch_assoc($result);

    if(mysqli_num_rows($result)>0){
        if($NewPassword == $row["NewPassword"]){
        ...
        <section class="flex-col py-36"  style="background-image: url('folder2/bgPng1.png');">
            ...
            <div class="flex justify-center py-20">
                <form action="" method="POST" autocomplete="off" class="flex-col">
                    ...
                    <input type="password" style="font-family: 'CharukolaUltraLight', sans-serif;" class="border-2 w-96 pl-4 border-slate-900 rounded-lg py-2 hover:scale-110 hover:border-[#c04848] ease-out duration-700" name="NewPassword" id="" placeholder="পাসওয়ার্ড"><br><br>
