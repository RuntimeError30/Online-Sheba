...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM hospitaldetails WHERE CONCAT(`Name`, `ID`, `Email`, `Division`, `Location`, `District`, `Mobile`, `RegistrationNumber`, `Password`, `ProfilePhoto`) LIKE '%".$valueToSearch."%'";
...
<body style="background-image: url('folder2/bgPng1.png');" class="bg-[#fffbff]">
    ...
    <main style="font-family: 'CharukolaUltraLight', sans-serif; " class="flex-col mx-16 px-9 py-2 my-11">
        ...
        <form action="" method="post">
            < <div class="flex">
                <input type="text" name="valueToSearch" id="search" value="" style="font-family: 'CharukolaUltraLight', sans-serif;" class="relative -top-5 bg-transparent border-2 w-96 h-11 pl-4 border-slate-900 rounded-lg py-2 hover:scale-110 hover:border-[#c04848] ease-out duration-700" placeholder="হাসপাতাল সার্চ করুন"><br>
                <input type="submit" name="search" value="হাসপাতাল সার্চ করুন" class="mx-4 relative -top-5 py-3 px-2 w-40 text-white bg-gradient-to-l from-[#c04848] to-[#480048] rounded-md hover:rounded-2xl hover:shadow-2xl hover:cursor-pointer hover:scale-110 hover:bg-gradient-to-r from-[#c04848] to-[#480048] transition-all ease-out duration-700">
                </div>
      ...
      <!-- populate table from mysql database -->
                <?php while($Hrow = mysqli_fetch_array($search_result)):?>
                <tr>
